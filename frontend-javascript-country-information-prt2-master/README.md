...
10. Maak een inputveld met zoek-knop op de pagina. In plaats van dat de data wordt opgehaald wanneer de pagina laadt,
    ...
    1. Maak een form na zoals op de screenshot foto (html & css)
    2. uitcomment de volgende functies in app (voor nu) zodat niet zichtbaar is behalve het form:
        - `const country = await API.getCountryByName('Netherland');`
        - `utils.renderHTMLForCountry(country);`
    3. Maak een JS functie in app.js (voor nu) die:
        - De gebruiker input uitleest zodra op enter gedrukt word (button click event of form submit)
        - [Zie goed voorbeeld](https://developer.mozilla.org/en-US/docs/Web/API/HTMLFormElement/submit_event#examples) 
        - Geef nu de gebruiker input door aan de functie `getCountryByName` om de data op te halen
        - Geef nu het resultaat van het ophalen van de data weer foor aan de functie `renderHTMLForCountry` om de data van het land te renderen
        - Tenslotte; verwijder de gebruiker input zodat de gebruiker opnieuw kan beginnen met het zoeken naar een land
